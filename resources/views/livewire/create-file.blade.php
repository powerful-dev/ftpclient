<div>
    ...
    <form wire:submit.prevent="save()">
        ...
        <div class="modal-header">
            <div>{{ __("Сreate File") }}</div>
            <a class="close" wire:click="$dispatch('modal.close')"><i class="bi bi-x"></i></a>
        </div>
        <div class="modal-body">
            <div class="form-group d-flex gap-2">
                <div class="l-12">
                    <label for="profile-name">{{ __("Enter File Name") }}</label>
                    <input id="focus-input" type="text" @if($error)class="error"@endif wire:model="name" placeholder="{{ __('Enter File Name') }}">
                </div>
            </div>
